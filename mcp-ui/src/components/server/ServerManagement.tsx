...
const ServerManagement: React.FC<ServerManagementProps> = ({ onError }) => {
  ...
  const handleExportToJson = async () => {
    try {
      setIsLoading(true);
      const configJson = await ServerApiService.exportToJson();
      ...
      if (onError) {
        onError(`配置导出失败: ${error instanceof Error ? error.message : '未知错误'}`);
      }
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="flex items-center justify-between p-6 border-b border-orange-400/20 bg-gradient-to-r from-slate-950/50 to-slate-900/50">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              disabled={isLoading}
              className="bg-purple-500/20 hover:bg-purple-500/30 border border-purple-500/50 text-purple-300 shadow-[0_0_10px_rgba(168,85,247,0.3)] px-4 py-2 rounded-lg transition-colors flex items-center space-x-2"
              title="导出所有服务器配置为JSON文件"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
